...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $insertOrderSQL = "INSERT INTO food_orders (total_price, user_id) VALUES (?, ?)";
        ...
        if (!empty($additionalProducts)) {
            ...
            for ($i = 0; $i < count($additionalProducts); $i++) {
                ...
                mysqli_stmt_bind_param($stmtAdditionalProduct, 'iiidd', $foodOrderID, $additionalProducts[$i], $additionalQuantities[$i], $additionalProductPrice, $totalAdditionalPrice);
...
function calculateProductPrice($productID)
{
    ...
    $query = "SELECT `food_price` FROM `foodcar_products` WHERE id = '$productID'";
